...
describe("GET /quotes?tag=:tag", () => {
    ...
    chai
      ...
      .end(async (err, res) => {
        try {
          ...
          console.log("res.body 2", (res.body, "response.body"));
          ...
          expect(res.body.data.length, "response.body.data.length").to.equal(
  ...
  it('should return an empty data array on no content query "foo"', async () => {
    const req = "/quotes";
    const res = await chai.request(app).get(req).query({ tag: "foo" });
    expect(res).to.have.status(200);
    expect(res).to.be.json;
    expect(res.body, "response.body").to.be.a("object");
    expect(res.body.data, "response.body.data").to.be.a("array");
